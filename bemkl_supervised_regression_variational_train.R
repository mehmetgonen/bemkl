...
bemkl_supervised_regression_variational_train <- function(Km, y, parameters) {    
  ...
  for (iter in 1:parameters$iteration) {
    ...
    if (parameters$progress == 1) {
      ...
      # p(a | lambda)
      lb <- lb - 0.5 * sum(as.vector(lambda$alpha * lambda$beta) * diag(atimesaT.mu)) - 0.5 * (D * log2pi - sum(log(lambda$alpha * lambda$beta)))
      # p(G | a, Km, upsilon)
      lb <- lb - 0.5 * sum(diag(GtimesGT.mu)) * upsilon$alpha * upsilon$beta + crossprod(a$mu, KmtimesGT.mu) * upsilon$alpha * upsilon$beta - 0.5 * sum(KmKm * atimesaT.mu) * upsilon$alpha * upsilon$beta - 0.5 * N * P * (log2pi - log(upsilon$alpha * upsilon$beta))
      # p(gamma)
      ...
      # p(b | gamma)
      lb <- lb - 0.5 * gamma$alpha * gamma$beta * btimesbT.mu - 0.5 * (log2pi - log(gamma$alpha * gamma$beta))
      # p(omega)
      ...
      # p(y | b, e, G, epsilon)
      lb <- lb - 0.5 * crossprod(y, y) * epsilon$alpha * epsilon$beta + crossprod(y, crossprod(G$mu, be$mu[2:(P + 1)])) * epsilon$alpha * epsilon$beta + sum(be$mu[1] * y) * epsilon$alpha * epsilon$beta - 0.5 * sum(etimeseT.mu * GtimesGT.mu) * epsilon$alpha * epsilon$beta - sum(crossprod(G$mu, etimesb.mu)) * epsilon$alpha * epsilon$beta - 0.5 * N * btimesbT.mu * epsilon$alpha * epsilon$beta - 0.5 * N * (log2pi - log(epsilon$alpha * epsilon$beta))
