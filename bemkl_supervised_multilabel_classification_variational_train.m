...
function state = bemkl_supervised_multilabel_classification_variational_train(Km, Y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        %%%% update b and e
        be.sigma = [diag(gamma.alpha .* gamma.beta) + N * eye(L, L), squeeze(sum(G.mu, 2))'; squeeze(sum(G.mu, 2)), diag(omega.alpha .* omega.beta)];
        for o = 1:L
        ...
        if parameters.progress == 1
            ...
            %%%% p(G | A, Km)
            for o = 1:L
                lb = lb - 0.5 * sigma_g^-2 * sum(diag(GtimesGT.mu(:, :, o))) + sigma_g^-2 * A.mu(:, o)' * KmtimesGT.mu(:, o) - 0.5 * sigma_g^-2 * sum(sum(KmKm .* atimesaT.mu(:, :, o))) - 0.5 * N * P * (log2pi + 2 * log(sigma_g));
            end
            %%%% p(gamma)
            ...
            %%%% p(F | b, e, G)
            for o = 1:L
                lb = lb - 0.5 * (F.mu(o, :) * F.mu(o, :)' + sum(F.sigma(o, :))) + F.mu(o, :) * (G.mu(:, :, o)' * be.mu(L + 1:L + P)) + sum(be.mu(o) * F.mu(o, :)) - 0.5 * sum(sum(etimeseT.mu .* GtimesGT.mu(:, :, o))) - sum(G.mu(:, :, o)' * etimesb.mu(:, o)) - 0.5 * N * btimesbT.mu(o, o) - 0.5 * N * log2pi;
            end
