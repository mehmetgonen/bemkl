...
function state = bemkl_supervised_regression_variational_train(Km, y, parameters)
    ...
    for iter = 1:parameters.iteration
        ...
        %%%% update upsilon        
        upsilon.beta = 1 / (1 / parameters.beta_upsilon + 0.5 * (sum(diag(GtimesGT.mu)) - 2 * sum(sum(reshape(a.mu' * Km, [N, P]) .* G.mu')) + sum(sum(KmKm .* atimesaT.mu))));
        %%%% update a
        ...
        %%%% update epsilon
        epsilon.beta = 1 / (1 / parameters.beta_epsilon + 0.5 * (y' * y - 2 * y' * [ones(1, N); G.mu]' * be.mu + N * btimesbT.mu + sum(sum(GtimesGT.mu .* etimeseT.mu)) + 2 * sum(G.mu, 2)' * etimesb.mu));
        %%%% update b and e
        be.sigma = [gamma.alpha * gamma.beta + epsilon.alpha * epsilon.beta * N, epsilon.alpha * epsilon.beta * sum(G.mu, 2)'; epsilon.alpha * epsilon.beta * sum(G.mu, 2), diag(omega.alpha .* omega.beta) + epsilon.alpha * epsilon.beta * (GtimesGT.mu)] \ eye(P + 1, P + 1);
        ...
        if parameters.progress == 1
            ...
            %%%% p(upsilon)
            lb = lb + (parameters.alpha_upsilon - 1) * (psi(upsilon.alpha) + log(upsilon.beta)) - upsilon.alpha * upsilon.beta / parameters.beta_upsilon - gammaln(parameters.alpha_upsilon) - parameters.alpha_upsilon * log(parameters.beta_upsilon);
            %%%% p(a | lambda)
            lb = lb - 0.5 * sum(lambda.alpha .* lambda.beta .* diag(atimesaT.mu)) - 0.5 * (D * log2pi - sum(psi(lambda.alpha) + log(lambda.beta)));
            %%%% p(G | a, Km, upsilon)
            lb = lb - 0.5 * sum(diag(GtimesGT.mu)) * upsilon.alpha * upsilon.beta + (a.mu' * KmtimesGT.mu) * upsilon.alpha * upsilon.beta - 0.5 * sum(sum(KmKm .* atimesaT.mu)) * upsilon.alpha * upsilon.beta - 0.5 * N * P * (log2pi - (psi(upsilon.alpha) + log(upsilon.beta)));
            %%%% p(gamma)
            ...
            %%%% p(epsilon)
            lb = lb + (parameters.alpha_epsilon - 1) * (psi(epsilon.alpha) + log(epsilon.beta)) - epsilon.alpha * epsilon.beta / parameters.beta_epsilon - gammaln(parameters.alpha_epsilon) - parameters.alpha_epsilon * log(parameters.beta_epsilon);
            %%%% p(y | b, e, G, epsilon)
            lb = lb - 0.5 * (y' * y) * epsilon.alpha * epsilon.beta + (y' * (G.mu' * be.mu(2:P + 1))) * epsilon.alpha * epsilon.beta + sum(be.mu(1) * y) * epsilon.alpha * epsilon.beta - 0.5 * sum(sum(etimeseT.mu .* GtimesGT.mu)) * epsilon.alpha * epsilon.beta - sum(G.mu' * etimesb.mu) * epsilon.alpha * epsilon.beta - 0.5 * N * btimesbT.mu * epsilon.alpha * epsilon.beta - 0.5 * N * (log2pi - (psi(epsilon.alpha) + log(epsilon.beta)));
